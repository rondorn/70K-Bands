...
//  70000TonsBands
//
//  SQLite-backed CSV importer for schedule/event data (via DataManager)
...
class ScheduleCSVImporter {
    ...
    /// Imports schedule data from CSV string into SQLite (THREAD-SAFE)
    ...
    func importEventsFromCSVString(_ csvString: String) -> Bool {
        // Import directly to SQLite (no Core Data)
        return importEventsFromCSVStringSynchronous(csvString)
    }
    
    /// Synchronous import implementation that writes to SQLite
    private func importEventsFromCSVStringSynchronous(_ csvString: String) -> Bool {
        print("DEBUG_MARKER: Starting schedule CSV import to SQLite")
    ...
    private func createOrUpdateEvent(
        ...
        do {
            ...
            // CRITICAL FIX: Also write to SQLite via DataManager
            // This ensures events are available in SQLite (the primary data layer)
            _ = dataManager.createOrUpdateEvent(
                bandName: band.bandName ?? "",
                timeIndex: timeIndex,
                endTimeIndex: endTimeIndex ?? 0,
                location: location,
                date: date,
                day: day,
                startTime: startTime,
            ...
            print("✅ [SQLITE_FIX] Event written to SQLite: \(band.bandName ?? "") at \(startTime ?? "")")
