...
class _SettingsSectionState extends State<SettingsSection> {
  ...
  Future<void> _addFestival() async {
    final result = await showDialog<_CreateFestivalResult>(
      context: context,
      barrierDismissible: false,
      builder: (context) => _CreateFestivalDialog(
...
class _CreateFestivalResult {
  const _CreateFestivalResult({
    required this.name,
...
class _CreateFestivalDialog extends StatefulWidget {
  const _CreateFestivalDialog({required this.dropboxConnected});

  final bool dropboxConnected;

  @override
  State<_CreateFestivalDialog> createState() => _CreateFestivalDialogState();
}

class _CreateFestivalDialogState extends State<_CreateFestivalDialog> {
  late final TextEditingController _name;
          ...
          child: Column(
            ...
            children: [
              ...
              if (!_createPointerFiles) ...[
                ...
                if (!widget.dropboxConnected)
                  const StatusBanner(
                    text: 'Connect Dropbox in Settings before creating files.',
                    isError: true,
                  ),
                TextField(
                  controller: _year,
