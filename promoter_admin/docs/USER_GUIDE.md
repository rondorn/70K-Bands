...
(Rare) On Mac/Windows first launch there is also **Use files on this computer only** — almost nobody needs this. Use it only if your festival **chooses not to use Dropbox** or **can no longer use Dropbox**. Details: [Appendix: Local File Mode](#appendix-local-file-mode-most-users-can-skip).
...
- **Local File Mode** *(Mac/Windows only; almost never needed)* — small button on the right of the button row. Skips Dropbox and uses files on this computer instead. **Most festivals ignore this.** Full details: [Appendix: Local File Mode](#appendix-local-file-mode-most-users-can-skip).  
...
| No **Local File Mode** button / other Local File Mode issues | Almost never needed — **Mac/Windows only.** See [Appendix: Local File Mode](#appendix-local-file-mode-most-users-can-skip). |
...
- [ ] *(Rare)* If the festival will not use Dropbox: see [Appendix: Local File Mode](#appendix-local-file-mode-most-users-can-skip) for folder paths, publishing, and pointer-file rules  
...
# Appendix: Local File Mode *(most users can skip)*

**You can ignore this entire appendix** unless one of the cases below applies. For almost every festival, **Dropbox remains the recommended and easiest solution** — stop here.

**When someone might use Local File Mode**

**Local File Mode is not recommended for normal use.** Use it only if:

- Your festival **chooses not to use Dropbox**, or  
- Your festival **can no longer use Dropbox**
...
On first launch (Mac/Windows), you can also choose **Use files on this computer only** instead of Connect Dropbox — same mode.
...
## Quick help (Local File Mode only)


| Symptom                                  | Likely fix                                                                                                                                                                                                                                                                                                                   |
| ---------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| No **Local File Mode** button            | **Mac and Windows only** — not on iPad or iPhone. Use normal Dropbox on mobile.                                                                                                                                                                                                                                              |
