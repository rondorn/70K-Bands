...
5. **Archives yearly app usage** — writes `{year}/{year}_App_Data.csv` (`Month, Highest Monthly Count`). Each run reads the current year file, leaves every other month unchanged, and updates the current month only if today's Platforms-tab total (trailing 30-day unique users) is higher than the stored value. The full report uses these files for a like-for-like year-over-year compare.
