...
class MasterViewController: UITableViewController, UISplitViewControllerDelegate, NSFetchedResultsControllerDelegate, UISearchBarDelegate {
    ...
    override func viewDidLoad() {
        ...
        // setNeedsStatusBarAppearanceUpdate() unavailable in visionOS - removed
    ...
    // didRotate method removed for visionOS compatibility - rotation handling is different in visionOS
    ...
    @IBAction func filterMenuButtonPress(_ sender: Any) {
        ...
        if (filterMenuButton.isHeld == false){
            createrFilterMenu(controller: self)
            // filterMenu.show() - DropDown functionality removed for visionOS compatibility
        } else {
            // filterMenu.hide() - DropDown functionality removed for visionOS compatibility
        }
    }
    ...
    // UITableViewRowAction removed for visionOS compatibility - swipe actions are handled differently in visionOS
    ...
    // UITableViewRowAction methods removed for visionOS compatibility
    ...
    func detailMenuChoices(cellDataText :String, bandName :String, segue :UIStoryboardSegue, indexPath: IndexPath) {
           ...
           if (cellData.count == 4 && getPromptForAttended() == true){
               ...
               // displayModeButtonItem unavailable in visionOS - removed
    ...
    @objc @IBAction func statsButtonTapped(_ sender: Any) {
        ...
        guard let documentsUrl = fileManager.urls(for: .documentDirectory, in: .userDomainMask).first else {
            self.showAlert("Error", message: "Could not access documents directory.")
            return
        }
